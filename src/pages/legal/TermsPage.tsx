    ...
    >
      ...
      <LegalSection id="servicio" index={3} title="Descripción del servicio">
        ...
        <LegalList
          items={[
            ...
            "Administrar la disponibilidad de una propiedad y sus recámaras. Al pausar un anuncio o una propiedad desde Mis anuncios, deja de aparecer en la búsqueda, pero quien tenga el enlace directo puede seguir abriéndolo y lo verá marcado como pausado (sin contacto). Al archivar, el anuncio deja de ser visible públicamente, incluso con el enlace directo. En publicaciones de propiedad, cada recámara se ofrece o se retira según su estado disponible/ocupada.",
            "Buscar y filtrar anuncios por ciudad, ubicación en mapa, precio y características. Algunos anuncios no muestran renta ni depósito: aparecen como Consultar $ y se omiten cuando ajustas un rango de precio distinto al predeterminado. Puedes guardar una búsqueda (filtros y zona) y, aparte, activar un aviso por correo; solo una búsqueda a la vez envía esos avisos. Abrir un enlace corto de búsqueda (anuncio, Facebook o mapa) muestra los cuartos que coinciden; no la guarda ni activa avisos hasta que eliges Guardar o Avisarme. Si editas una búsqueda compartida, se crea una copia personal con otra dirección; el enlace original no cambia. Los avisos por correo de búsquedas no se envían entre las 23:00 y las 6:00 hora local de la ciudad de esa búsqueda (reanudamos a las 6:01).",
            "Escribir al número de WhatsApp Business de Bestie para buscar anuncios en Guadalajara (eliges una zona y te enviamos de inmediato los anuncios que coinciden, más un recuento de lo que hay cerca y en toda la ciudad; después puedes ajustar presupuesto o preferencia de roomies) o para publicar un anuncio de un solo cuarto desde el mismo chat: puedes enviar hasta dos infográficos (una plantilla que junta información del cuarto o la propiedad, a veces con fotos; la IA los lee para extraer datos y también se incluyen en la galería pública junto con las fotos del espacio), fotos del espacio (opcionales: puedes publicar sin ellas y completarlas después; si publicas sin fotos te avisamos en la app y, si corresponde, por correo o SMS con un enlace para subirlas), una descripción opcional, la renta mensual exacta, el tipo y tamaño de recámara, la preferencia de roomies, el depósito, las etiquetas de lo que incluye, una ubicación aproximada (radio de privacidad) y un toque de aceptación de estos Términos y del Aviso de Privacidad antes de que el anuncio quede público. El chat solo pregunta los datos que no pudo deducir de tu descripción o infográfico. Si no escribes descripción, la IA redacta una con esos mismos datos y la ves en la vista previa antes de publicar. Las respuestas en ese chat son automáticas. El celular mexicano desde el que escribes es tu cuenta en Bestie (si aún no tenías una, la creamos al usar el canal). Conservamos ese número, las fotos, los infográficos, la ubicación aproximada y el contenido del hilo para operar el canal. Publicar varias recámaras de una misma propiedad se hace en el sitio. Messenger, si está disponible, usa el mismo buscador guiado por zona (con ayuda de IA para leer lo que escribes) y no publica desde el chat: te damos un enlace para terminar el anuncio en el sitio.",
            "Contactar a otros usuarios mediante el chat directo en Bestie o, si el anuncio lo habilita y aceptas el aviso de seguridad, revelar el teléfono, llamar o abrir WhatsApp. Registramos esas acciones y, si quien publica tiene cuenta en Bestie, le avisamos en la app con tu nombre visible, el anuncio y el tipo de interés (consulta del número, llamada o WhatsApp). El equipo operador de Bestie recibe el mismo tipo de aviso en la app (también cuando envías el primer mensaje de un anuncio) para seguimiento operativo, sin incluir el contenido del chat. Si quien publica tiene teléfono verificado y avisos activos, también podemos enviarle un SMS transaccional la primera vez que una persona le escribe por un anuncio, para que revise el correo de Bestie.mx (incluida la carpeta de spam) o abra Mensajes en la Plataforma.",
            "Crear y administrar un perfil de usuario.",
            "Escribir a Soporte de Bestie desde un chat directo en la app (sección Contacto) o desde el botón de ayuda en el mapa de búsqueda, para comentarios, preguntas sobre el producto o solicitudes, con la posibilidad de adjuntar imágenes. Este chat requiere iniciar sesión y las respuestas pueden tardar hasta 48 horas. Cuando tengas mensajes nuevos (incluido Soporte o Feedback), podemos enviarte un correo de aviso (como máximo uno cada 3 horas), sin incluir el contenido de los mensajes. No enviamos esos correos ni los SMS de aviso de mensajes entre las 23:00 y las 6:00 hora local del anuncio (reanudamos a las 6:01); el intervalo de 3 horas no acumula varios envíos durante esa ventana. Tras publicar un anuncio sin fotos, también podemos enviarte un aviso en la app y, si aplica, un correo o SMS con un enlace para subirlas (mismos horarios de silencio).",
      ...
      <LegalSection id="elegibilidad" index={4} title="Elegibilidad y cuenta">
        <LegalList
          items={[
            ...
            "Puedes crear tu cuenta con un correo electrónico o con un celular mexicano (+52) verificado por SMS, más una contraseña. En el registro por celular puedes agregar una foto de perfil opcional. El número de perfil usado para entrar debe ser de México; los teléfonos de contacto en un anuncio pueden tener otro código de país y no se verifican por SMS. Si escribes al WhatsApp Business de Bestie desde un celular mexicano, Meta ya comprobó ese número: lo usamos como tu cuenta (o lo ligamos a la cuenta Bestie que ya tenga ese celular). Puedes completar correo y contraseña después en el sitio.",
      ...
      <LegalSection id="rol" index={8} title="Rol de Bestie e intermediación">
        ...
        <LegalList
          items={[
            ...
            "Antes de ver mensajes entre usuarios o de revelar el teléfono asociado a un anuncio, puedes recibir un aviso de seguridad en la app. Al aceptarlo, reconoces el riesgo de estafas o fraudes entre usuarios y que Bestie y el Titular no son responsables por el uso del chat, llamadas, SMS, WhatsApp ni por acuerdos o pagos entre ustedes. Registramos esa aceptación (fecha y versión del aviso) como evidencia de tu consentimiento. Si revelas el teléfono, llamas o abres WhatsApp hacia el número de un anuncio, o envías el primer mensaje en el chat de un anuncio, también registramos esa acción; si quien publica tiene cuenta en Bestie, le mostramos una notificación en la app con tu nombre visible y el título del anuncio, y el equipo operador de Bestie recibe el mismo tipo de aviso en la app para seguimiento operativo (sin el contenido del chat).",
          ]}
        />
      </LegalSection>
      ...
      <LegalSection id="terceros" index={10} title="Servicios de terceros">
        <p>
          ...
          envío de correos electrónicos, envío de SMS de verificación, de aviso transaccional (primer mensaje en un anuncio) y de confirmación transaccional a
