...
const ACCOUNT_TABS = [
  { to: "/mis-anuncios", label: "Mis Anuncios", end: true, mobile: true },
  { to: "/mis-busquedas", label: "Mis Búsquedas", end: true, mobile: true },
  { to: "/mensajes", label: "Mensajes", end: true, mobile: true },
  { to: "/contacto", label: "Contacto", end: true, mobile: false },
  { to: "/perfil", label: "Mi Perfil", end: false, mobile: false },
] as const;
...
export function AccountNavTabs({ unreadMessageCount = 0 }: AccountNavTabsProps) {
  ...
  return (
    ...
    >
      <div
        role="tablist"
        className="-mb-px flex justify-center gap-1 overflow-x-auto overscroll-x-contain px-4 pb-px sm:px-6 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
      >
        {ACCOUNT_TABS.map((tab) => {
          ...
          return (
            <NavLink
              ...
              className={`relative min-h-11 shrink-0 items-center gap-1.5 whitespace-nowrap border-b-2 px-3 text-sm font-semibold transition sm:px-4 ${
                tab.mobile ? "inline-flex" : "hidden md:inline-flex"
              } ${
                active
