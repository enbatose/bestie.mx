...
export function SearchViewAllCta({
  ...
  const label = expanding ? `Ver todas (${total})` : "Volver a mi búsqueda";
  ...
  const button = (
    <button
      ...
      className={`inline-flex items-center justify-center gap-1.5 rounded-full font-semibold shadow-sm transition active:scale-[0.99] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-secondary/40 ${buttonTone} ${
        layout === "bar"
          ? "min-h-9 shrink-0 px-3 text-xs sm:min-h-10 sm:px-4 sm:text-sm"
          : "min-h-11 w-full min-w-0 px-4 text-sm"
      }`}
    >
  ...
  return (
    <div className="flex min-w-0 flex-1 items-center gap-2">
      <p className="min-w-0 flex-1 break-words text-[0.7rem] leading-snug text-muted sm:text-xs">
        {hint}
      </p>
      {button}
    </div>
