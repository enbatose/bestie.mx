...
function roomCoverSrc(room: Room): string | null {
  const first = (room.imageUrls ?? room.photos ?? []).find((url) => url.trim());
        ...
        <div id="property-available-rooms" className="scroll-mt-24">
          ...
          <div className="mt-3 grid gap-3 md:grid-cols-2">
            {availableRooms.map((room) => {
              const coverSrc = roomCoverSrc(room);
              const roomLabel = room.customName || room.title;
              return (
                <article
                  id={`available-room-${room.id}`}
                  key={room.id}
                  className="rounded-xl border border-border bg-bg-light p-4"
                >
                  <div className="flex gap-3">
                    ...
                    <div
                      className="relative aspect-square w-24 shrink-0 overflow-hidden rounded-xl bg-surface ring-1 ring-border sm:w-28"
                      aria-hidden={!coverSrc}
                    >
                      {coverSrc ? (
                        <img
                          src={coverSrc}
                          alt={`Foto de ${roomLabel}`}
