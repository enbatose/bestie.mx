...
- **PostHog:** set `VITE_POSTHOG_PROJECT_TOKEN` on **bestie-prod only**. Declare matching `ARG`/`ENV` in the root `Dockerfile` (already done for PostHog + Maps) so Docker builds bake the token — otherwise Prod ships an empty token and session replay never starts. The client also refuses to init outside `bestie.mx` / `www.bestie.mx`.
